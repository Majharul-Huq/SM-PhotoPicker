...
- (void)setupSubviews
{
    ...
    if (count == 2 || count == 4) {
        ...
        for (JKPopMenuItem *item in self.menuItems) {
            ...
            switch (x_index) {
                case 0:
                    item.animationTime = 0.0 + 0.01*y_index;
                    break;
                case 1:
                    item.animationTime = 0.1 + 0.01*y_index;
                    break;
            }
            item.tag = index;
...
- (void)appear
{
    ...
    if ([self.delegate respondsToSelector:@selector(popMenuDidAppeared)]) {
        [self.delegate popMenuDidAppeared];
    }
...
- (void)disappear
{
    ...
    if ([self.delegate respondsToSelector:@selector(popMenuDidDisappeared)]) {
        [self.delegate popMenuDidDisappeared];
    }
